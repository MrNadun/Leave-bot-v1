/**********************************************************************
  Leave-bot-v1 | case.js 
  CODE FIXED, ERRORS REMOVED & LOGIC IMPROVED | 2025-11
**********************************************************************/
...
module.exports = async (m, conn, from, store) => {
  ...
  const onlygc = () => {
    conn.sendMessage(m.chat, {
      text: `_Halo ${pushname}, *xbot-next version* hanya bisa digunakan di dalam group saja. Jika ingin menggunakan, silakan invite ke group!_`,
      contextInfo: {
        externalAdReply: {
...
💡 LOCATION : ${conn.groupName}`));
  ...
  switch (order) {
    ...
    {
      let help =
`👋 Hello *${pushname}*, Here's how to use me!\n
╭──☉ 📜 Menu
│┬🍀 .leave       <Request a leave>
│┬🍀 .halfleave   <Request half leave>
│┬🍀 .privat      <Work only private>
│┬🍀 .public      <Work only public>
│┬🍀 .hidetag     Tag group members invisibly
│┬🍀 .tagall      Tag all group members
│┬🍀 .vv          Read view once msg
╰─────────────☉`;
      conn.sendMessage(m.chat, {
        document: fs.readFileSync('./img.jpg'),
        fileName: 'menu.jpg',
...
`;
      let sentMsg = await conn.sendMessage(m.chat, {
        document: fs.readFileSync('./img.jpg'),
        fileName: 'half-leave-request.jpg',
        mimetype: 'image/jpeg',
